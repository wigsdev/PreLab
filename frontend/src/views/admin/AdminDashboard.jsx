        ...
        <div className="max-w-6xl mx-auto px-4 py-8 pb-24">
            <header className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Resumen global de la plataforma</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                ...
                <StatCard
                    ...
                    bg="bg-purple-500/10"
                    trend={`+${stats.exams_this_week} esta semana`}
                />
                <StatCard
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                                <Activity size={20} className="text-indigo-500" /> Tendencia de Exámenes
                            </h3>
                            <span className="text-xs text-slate-500 font-medium bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg">Últimos 7 días</span>
                        </div>
                                ...
                                return (
                                    <div key={idx} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer">
                                        <div className="relative w-full flex justify-center">
                                            ...
                                            {/* Bar */}
                                            <div
                                                className="w-full max-w-[40px] bg-indigo-100 dark:bg-indigo-900/30 rounded-t-lg group-hover:bg-indigo-500 dark:group-hover:bg-indigo-500 transition-all relative overflow-hidden"
                                                style={{ height: `${Math.max(height, 5)}%` }} // Min height for visual
                                            >
                                                {height > 0 && (
                                                    <div
                                        ...
                                        <span className="text-[10px] sm:text-xs text-slate-400 font-medium text-center">{item.date}</span>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Actividad Reciente</h3>
                            <button className="text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Ver todo</button>
                        </div>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {stats.recent_activity.map((item) => (
                                        <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td className="p-4">
                                                <span className={`font-bold ${item.score >= 11 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'
                                                    }`}>
                                                    {item.score}
                                                </span>
                                            </td>
                            ...
                            {stats.top_students.map((student, idx) => (
                                <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-slate-800 dark:text-white">{student.name}</p>
                                            <p className="text-xs text-slate-500">{student.exams} exámenes</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <span className="font-black text-indigo-600 dark:text-indigo-400">{student.avg_score}</span>
                                        <p className="text-[10px] text-slate-400 uppercase font-bold">Promedio</p>
                                    </div>
                                </div>
                            ))}
                            {stats.top_students.length === 0 && (
                                <p className="text-slate-500 text-sm text-center py-4">Aún no hay datos suficientes</p>
                            )}
                        </div>
                    </div>
                </div>
